...
export default function AddActivityForm({ onClose }: AddActivityFormProps) {
  ...
  const { addCustomActivity } = useApp();
  ...
  const handleAddActivity = async () => {
    ...
    const newActivity = {
      ...
      category: "daily",
      niyyahText:
        newNiyyah.trim() || t("reminders.defaultIntention", { activity: newName.trim() }),
      niyyahTextAr: lang === "ar" ? newNiyyah.trim() : "",
      color: selectedColor,
      enabled: true,
    };

    // UserActivity fromAppContext doesn't need enabled when adding custom, but we can pass it or it will be handled by addCustomActivity based on the function signature.
    // Let's pass it strictly as the previous code did:
    const { enabled, ...activityRest } = newActivity;
    await addCustomActivity(activityRest);
